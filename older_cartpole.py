        ...
        while done == False:
            ...
            if done:
                ...
                if batch_counter == batch_size:
                    ...
                    action_tensor_m = torch.unsqueeze(action_tensor, 1)
                    action_tensor_m = F.pad(input=action_tensor_m, pad=(0, 1,), mode='constant', value=0)
                    
                    #print(torch.gather(logprob, 1, action_tensor_m))
                    selected_logprobs = torch.gather(logprob, 1, action_tensor_m)[:, 0].squeeze()
                    #print(selected_logprobs)
                    
                    #print(f"{reward_tensor.shape=}, {selected_logprobs.shape=}")
                ...
                print("\rEp: {0} Average of last 100:" +   
                     "{:.2f}".format(
